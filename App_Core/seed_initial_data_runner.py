...
def _write_success(stdout, style, msg: str) -> None:
    if stdout is None:
        return
    if style:
...
def _write_line(stdout, msg: str) -> None:
    if stdout is None:
        return
    stdout.write(msg + '\n')


def _sync_access(user, store_names, default_store_name, store_map):
...
def _seed_pending_qr_orders(*, tenant, store_map):
...
def add_seed_initial_data_arguments(parser):
    """Dùng chung cho Command.add_arguments."""
...
def run_seed_initial_data(
    *,
    ...
    """
    Seed idempotent cho tenant (mặc định demo).
    Xóa ảnh upload sản phẩm (trả về image_url mẫu), xóa QR thanh toán cửa hàng seed, reset mật khẩu khi reset_passwords.
    """
    seed_qr_pending = seed_qr_pending and not skip_qr_pending
    ...
    for row in stores_seed:
        ...
        if store.payment_qr:
            store.payment_qr.delete(save=False)
        store.payment_qr = None
        store.payment_bank_name = ''
        store.payment_account_name = ''
        store.payment_account_number = ''
        store.save(
            update_fields=[
                'address',
                'is_active',
    ...
    _sync_access(manager, [row['name'] for row in stores_seed], 'CN Trung Tâm', store_map)
    for idx, staff in enumerate(staff_users):
        _sync_access(staff, staff_rows[idx][1], staff_rows[idx][2], store_map)
    ...
    for row in product_templates:
        ...
        clear_product_uploaded_images(product)
